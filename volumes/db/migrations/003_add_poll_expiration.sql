...
--     PERFORM cron.schedule('poll-cleanup', '*/30 * * * *', 'SELECT cleanup_expired_polls_with_log(''pg_cron'');');
--   END IF;
-- END $$;
...
COMMENT ON FUNCTION cleanup_expired_polls_with_log(TEXT) IS 'Enhanced cleanup function with logging support for tracking cleanup operations.';
COMMENT ON FUNCTION periodic_poll_cleanup() IS 'Runs cleanup if more than 30 minutes have passed since last cleanup.';
