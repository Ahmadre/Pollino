...
class _PollCardState extends State<_PollCard> {
  ...
  void initState() {
    ...
    if (widget.poll.pollType != 'FEEDBACK') {
      ...
      _loadAiSummaryPreview();
    }
  }

  Future<void> _loadAiSummaryPreview() async {
    try {
      final data =
          await ApiService.getPublicAiSummary(widget.poll.id.toString());
      if (mounted && data['available'] == true) {
        final raw = (data['summaryText'] as String? ?? '').trim();
        // Strip markdown headings/bold for clean preview
        final clean = raw
...
// Vote / response count and expiration info
            ...
            // AI summary preview for FEEDBACK polls
            if (widget.poll.pollType == 'FEEDBACK' &&
                _aiSummaryPreview != null) ...[
              const SizedBox(height: 12),
              Container(
                width: double.infinity,
                padding:
